<template>
  <div>
    <el-dialog v-model="userStore.loginVisible" title="用户登录">
      <div class="login">
        <div class="left">
          <div v-show="sence === 0">
            <el-form>
              <el-form-item>
                <el-input
                  placeholder="请输入手机号码"
                  :prefix-icon="User"
                  v-model="loginParam.phone"
                />
              </el-form-item>
              <el-form-item>
                <el-input
                  placeholder="请输入密码"
                  :prefix-icon="Lock"
                  v-model="loginParam.password"
                />
              </el-form-item>
              <el-form-item>
                <el-button :disabled="!isPhone || flag" @click="getCode"
                  >
                  <span v-if="!flag">获取验证码</span>
                  <span v-else>{{ `获取验证码 ${time} s` }}</span>
                  </el-button
                >
              </el-form-item>
            </el-form>
            <div class="login-btn">
              <el-button style="width: 100%" type="primary">用户登录</el-button>
              <div class="wx">
                <p>微信扫码登录</p>
                <svg
                  t="1692512123897"
                  class="icon"
                  viewBox="0 0 1024 1024"
                  version="1.1"
                  xmlns="http://www.w3.org/2000/svg"
                  p-id="5598"
                  width="32"
                  height="32"
                >
                  <path
                    d="M337.387283 341.82659c-17.757225 0-35.514451 11.83815-35.514451 29.595375s17.757225 29.595376 35.514451 29.595376 29.595376-11.83815 29.595376-29.595376c0-18.49711-11.83815-29.595376-29.595376-29.595375zM577.849711 513.479769c-11.83815 0-22.936416 12.578035-22.936416 23.6763 0 12.578035 11.83815 23.676301 22.936416 23.676301 17.757225 0 29.595376-11.83815 29.595376-23.676301s-11.83815-23.676301-29.595376-23.6763zM501.641618 401.017341c17.757225 0 29.595376-12.578035 29.595376-29.595376 0-17.757225-11.83815-29.595376-29.595376-29.595375s-35.514451 11.83815-35.51445 29.595375 17.757225 29.595376 35.51445 29.595376zM706.589595 513.479769c-11.83815 0-22.936416 12.578035-22.936416 23.6763 0 12.578035 11.83815 23.676301 22.936416 23.676301 17.757225 0 29.595376-11.83815 29.595376-23.676301s-11.83815-23.676301-29.595376-23.6763z"
                    fill="#28C445"
                    p-id="5599"
                  ></path>
                  <path
                    d="M510.520231 2.959538C228.624277 2.959538 0 231.583815 0 513.479769s228.624277 510.520231 510.520231 510.520231 510.520231-228.624277 510.520231-510.520231-228.624277-510.520231-510.520231-510.520231zM413.595376 644.439306c-29.595376 0-53.271676-5.919075-81.387284-12.578034l-81.387283 41.433526 22.936416-71.768786c-58.450867-41.433526-93.965318-95.445087-93.965317-159.815029 0-113.202312 105.803468-201.988439 233.803468-201.98844 114.682081 0 216.046243 71.028902 236.023121 166.473989-7.398844-0.739884-14.797688-1.479769-22.196532-1.479769-110.982659 1.479769-198.289017 85.086705-198.289017 188.67052 0 17.017341 2.959538 33.294798 7.398844 49.572255-7.398844 0.739884-15.537572 1.479769-22.936416 1.479768z m346.265896 82.867052l17.757225 59.190752-63.630058-35.514451c-22.936416 5.919075-46.612717 11.83815-70.289017 11.83815-111.722543 0-199.768786-76.947977-199.768786-172.393063-0.739884-94.705202 87.306358-171.653179 198.289017-171.65318 105.803468 0 199.028902 77.687861 199.028902 172.393064 0 53.271676-34.774566 100.624277-81.387283 136.138728z"
                    fill="#28C445"
                    p-id="5600"
                  ></path>
                </svg>
              </div>
            </div>
          </div>
        </div>
        <div class="right">
          <div class="top">
            <div class="vx-left">
              <svg
                t="1692512538087"
                class="icon"
                viewBox="0 0 1024 1024"
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                p-id="6742"
                width="150"
                height="150"
              >
                <path
                  d="M418.037 538.846H64.56a63.92 63.92 0 0 0-63.92 63.92v356.035a63.92 63.92 0 0 0 63.92 63.92h353.478a63.92 63.92 0 0 0 63.92-63.92V602.767a63.92 63.92 0 0 0-63.92-63.92z m0 419.955H64.56V602.767h353.478z"
                  p-id="6743"
                ></path>
                <path
                  d="M145.418 684.904h191.76v191.76h-191.76zM418.038 0H64.558A63.92 63.92 0 0 0 0.64 63.92v347.086a63.92 63.92 0 0 0 63.92 63.92h353.478a63.92 63.92 0 0 0 63.92-63.92V63.92A63.92 63.92 0 0 0 418.038 0z m0 411.006H64.558V63.92h353.478z"
                  p-id="6744"
                ></path>
                <path
                  d="M145.418 141.583h191.76v191.76h-191.76z m845.983 396.305H577.518a31.96 31.96 0 0 0-29.723 40.589v412.285a31.96 31.96 0 0 0 63.92 0V601.808h379.686a31.96 31.96 0 0 0 0-63.92zM959.44 0H609.798a63.92 63.92 0 0 0-63.92 63.92v347.086a63.92 63.92 0 0 0 63.92 63.92H959.44a63.92 63.92 0 0 0 63.92-63.92V63.92A63.92 63.92 0 0 0 959.44 0z m0 411.006H609.798V63.92H959.44z"
                  p-id="6745"
                ></path>
                <path
                  d="M688.74 141.583H880.5v191.76H688.74z m250.885 527.021h82.457v82.457h-82.457z m-220.204 0h-82.457v82.457h82.457v162.996h82.457V750.742H719.42v-82.138z m-82.457 272.62h82.457v82.456h-82.457z m181.533-231.71h82.457v82.456h-82.457z m-65.838-94.922h82.457v82.457H752.66zM815.62 936.11h82.457v82.457H815.62z m124.005-55.93h82.457V1024h-82.457z"
                  p-id="6746"
                ></path>
              </svg>
              <svg
                t="1692512583055"
                class="icon"
                viewBox="0 0 1024 1024"
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                p-id="7941"
                width="32"
                height="32"
              >
                <path
                  d="M664.250054 368.541681c10.015098 0 19.892049 0.732687 29.67281 1.795902-26.647917-122.810047-159.358451-214.077703-310.826188-214.077703-169.353083 0-308.085774 114.232694-308.085774 259.274068 0 83.708494 46.165436 152.460344 123.281791 205.78483l-30.80868 91.730191 107.688651-53.455469c38.558178 7.53665 69.459978 15.308661 107.924012 15.308661 9.66308 0 19.230993-0.470721 28.752858-1.225921-6.025227-20.36584-9.521864-41.723264-9.521864-63.862493C402.328693 476.632491 517.908058 368.541681 664.250054 368.541681zM498.62897 285.87389c23.200398 0 38.557154 15.120372 38.557154 38.061874 0 22.846334-15.356756 38.156018-38.557154 38.156018-23.107277 0-46.260603-15.309684-46.260603-38.156018C452.368366 300.994262 475.522716 285.87389 498.62897 285.87389zM283.016307 362.090758c-23.107277 0-46.402843-15.309684-46.402843-38.156018 0-22.941502 23.295566-38.061874 46.402843-38.061874 23.081695 0 38.46301 15.120372 38.46301 38.061874C321.479317 346.782098 306.098002 362.090758 283.016307 362.090758zM945.448458 606.151333c0-121.888048-123.258255-221.236753-261.683954-221.236753-146.57838 0-262.015505 99.348706-262.015505 221.236753 0 122.06508 115.437126 221.200938 262.015505 221.200938 30.66644 0 61.617359-7.609305 92.423993-15.262612l84.513836 45.786813-23.178909-76.17082C899.379213 735.776599 945.448458 674.90216 945.448458 606.151333zM598.803483 567.994292c-15.332197 0-30.807656-15.096836-30.807656-30.501688 0-15.190981 15.47546-30.477129 30.807656-30.477129 23.295566 0 38.558178 15.286148 38.558178 30.477129C637.361661 552.897456 622.099049 567.994292 598.803483 567.994292zM768.25071 567.994292c-15.213493 0-30.594809-15.096836-30.594809-30.501688 0-15.190981 15.381315-30.477129 30.594809-30.477129 23.107277 0 38.558178 15.286148 38.558178 30.477129C806.808888 552.897456 791.357987 567.994292 768.25071 567.994292z"
                  fill="#5D5D5D"
                  p-id="7942"
                ></path>
              </svg>
              <p>微信扫一扫关注</p>
              <p>“快速预约挂号”</p>
            </div>
            <div class="phone-right">
              <svg
                t="1692512538087"
                class="icon"
                viewBox="0 0 1024 1024"
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                p-id="6742"
                width="150"
                height="150"
              >
                <path
                  d="M418.037 538.846H64.56a63.92 63.92 0 0 0-63.92 63.92v356.035a63.92 63.92 0 0 0 63.92 63.92h353.478a63.92 63.92 0 0 0 63.92-63.92V602.767a63.92 63.92 0 0 0-63.92-63.92z m0 419.955H64.56V602.767h353.478z"
                  p-id="6743"
                ></path>
                <path
                  d="M145.418 684.904h191.76v191.76h-191.76zM418.038 0H64.558A63.92 63.92 0 0 0 0.64 63.92v347.086a63.92 63.92 0 0 0 63.92 63.92h353.478a63.92 63.92 0 0 0 63.92-63.92V63.92A63.92 63.92 0 0 0 418.038 0z m0 411.006H64.558V63.92h353.478z"
                  p-id="6744"
                ></path>
                <path
                  d="M145.418 141.583h191.76v191.76h-191.76z m845.983 396.305H577.518a31.96 31.96 0 0 0-29.723 40.589v412.285a31.96 31.96 0 0 0 63.92 0V601.808h379.686a31.96 31.96 0 0 0 0-63.92zM959.44 0H609.798a63.92 63.92 0 0 0-63.92 63.92v347.086a63.92 63.92 0 0 0 63.92 63.92H959.44a63.92 63.92 0 0 0 63.92-63.92V63.92A63.92 63.92 0 0 0 959.44 0z m0 411.006H609.798V63.92H959.44z"
                  p-id="6745"
                ></path>
                <path
                  d="M688.74 141.583H880.5v191.76H688.74z m250.885 527.021h82.457v82.457h-82.457z m-220.204 0h-82.457v82.457h82.457v162.996h82.457V750.742H719.42v-82.138z m-82.457 272.62h82.457v82.456h-82.457z m181.533-231.71h82.457v82.456h-82.457z m-65.838-94.922h82.457v82.457H752.66zM815.62 936.11h82.457v82.457H815.62z m124.005-55.93h82.457V1024h-82.457z"
                  p-id="6746"
                ></path>
              </svg>
              <svg
                t="1692512661089"
                class="icon"
                viewBox="0 0 1024 1024"
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                p-id="9033"
                width="32"
                height="32"
              >
                <path
                  d="M747.6 139H275c-21.8 0-39.4 17.6-39.4 39.4V848c0 21.8 17.6 39.4 39.4 39.4h472.6c21.8 0 39.4-17.6 39.4-39.4V178.4c0.1-21.7-17.6-39.4-39.4-39.4z m-19.7 689.3H294.7V198.1h433.2v630.2z"
                  fill="#666666"
                  p-id="9034"
                ></path>
                <path
                  d="M373.5 631.4h275.7c21.7 0 39.4-17.6 39.4-39.4V276.9c0-21.7-17.6-39.4-39.4-39.4H373.5c-21.7 0-39.4 17.6-39.4 39.4V592c0 21.7 17.7 39.4 39.4 39.4z m19.7-334.8h236.3v275.7H393.2V296.6zM511.4 670.8c-32.6 0-59.1 26.4-59.1 59.1s26.5 59.1 59.1 59.1 59.1-26.4 59.1-59.1-26.5-59.1-59.1-59.1z"
                  fill="#666666"
                  p-id="9035"
                ></path>
              </svg>
              <p>扫一扫下载</p>
              <p>“预约挂号”App</p>
            </div>
          </div>
          <div class="bottom">
            <p>医院官方指定平台</p>
            <p>快速挂号 安全放心</p>
          </div>
        </div>
      </div>
    </el-dialog>
  </div>
</template>

<script setup lang="ts">
import { User, Lock } from '@element-plus/icons-vue'
import useUserStore from '@/store/module/user'
import { ref, reactive, computed,watch } from 'vue'
import { getSmsCode} from '@/api/user'
let userStore = useUserStore()
const loginParam = reactive({
  phone: '',
  password: '',
})
let time =ref<number>(5)
let flag =ref<boolean>(false)
let sence = ref<number>(0)
let isPhone = computed(() => {
  //手机正则
  const phoneReg = /^1[3456789]\d{9}$/
  return phoneReg.test(loginParam.phone)
})
watch(()=> flag.value,(val)=>{
  if(val){
    let timer = setInterval(()=>{
      time.value--
      if(time.value<=0){
        clearInterval(timer)
        time.value = 5
        flag.value = false
      }
    },1000)
  }
})
const getCode =async () => {
  const res:any = await getSmsCode(loginParam.phone)
  if(res.code === 200){
    loginParam.password = res.data
    flag.value = true
  }
}
</script>

<style scoped lang="scss">
.login {
  display: flex;
  .left {
    padding: 32px;
    border: 1px solid #dcdfe6;
    width: 50%;
    .login-btn {
      display: flex;
      flex-direction: column;
      .wx {
        cursor: pointer;
        font-size: 14px;
        display: flex;
        flex-direction: column;
        align-items: center;
        p {
          margin: 8px 0;
        }
      }
    }
  }
  .right {
    flex: 1;
    display: flex;
    flex-direction: column;
    .top {
      display: flex;
      justify-content: space-around;
      .vx-left,
      .phone-right {
        display: flex;
        flex-direction: column;
        align-items: center;
      }
    }
    .bottom {
      margin-top: 32px;
      text-align: center;
      font-size: 22px;
      font-weight: 700;
      color: #333;
      p {
        margin: 8px 0;
      }
    }
  }
}
</style>
